...
function rebuildUserItemMaps(fromArray){
  user2items = new Map();
  item2users = new Map();
  for (const {u,i,r,ts} of fromArray){
  ...
  try{
    ...
    const tried = {
      recipes: [
        ...
        './recipes.csv','recipes.csv','data/recipes.csv',
        './RAW_recipes.csv','RAW_recipes.csv','data/RAW_recipes.csv'
      ],
      train:   ['./interactions_train.csv','interactions_train.csv','data/interactions_train.csv','./interactions.csv','interactions.csv','data/interactions.csv'],
    ...
    parseInteractions(va?.text ?? '', valid);

    // collect full user set
    for (const x of train) users.add(x.u);
    for (const x of valid) users.add(x.u);
    ...
    // rebuild maps on the final train only (used by demo and graph)
    rebuildUserItemMaps(train);
    buildIndexers();
    buildTagVocab(parseInt(($('dK')?.value||'200'),10));

    const density = (train.length / (users.size * Math.max(1, items.size))).toExponential(2);
    const coldUsers = Array.from((function(){
      const m = new Map(); for (const r of train){ m.set(r.u,(m.get(r.u)||0)+1); } return m;
    })().values()).filter(c=>c<5).length;
    const itemCnts = (function(){
      const m = new Map(); for (const r of train){ m.set(r.i,(m.get(r.i)||0)+1); } return m;
    })();
    const coldItems = Array.from(items.keys()).filter(i=>!itemCnts.has(i) || itemCnts.get(i)<5).length;
...
async function demoOnce(){
  ...
  // Safe render: only rows with finite scores
  const render = (tblId, scoresArr) => {
    ...
    const picked = candIdx
      .map(ii=>({ii, s:Number(scoresArr[ii])}))
...
async function computeAndShowMetrics(){
  ...
  body.textContent = 'Computing metrics…';
  ...
  async function evalModel(model){
    ...
    let hits=0, dcg=0, ideal=0, usedUsers=0;
    for (const u of uPick){
      ...
      // map to item indices and drop anything missing
      const candIdx = candidates.map(i=>itemIndex.get(i)).filter(Number.isInteger);
      if (!candIdx.length) continue;

      const uIdx = tf.tensor1d([userIndex.get(u)], 'int32');
      const scores = await model.scoreItems(uIdx, candIdx);
      uIdx.dispose();
      const arr = Array.from(scores.dataSync()); scores.dispose();

      // zip back with relevance labels (some items may have been dropped)
      const zipped = candIdx.map((ii,ix)=>({
        ii, s: arr[ix], rel: testPos.includes(idx2item[ii]) ? 1 : 0
      })).sort((a,b)=>b.s-a.s).slice(0,K);

      if (!zipped.length) continue;
      usedUsers++;
    ...
    if (!usedUsers) return { recallAt10: 0, ndcgAt10: 0, usedUsers: 0 };
    return { recallAt10: hits/Math.max(1,ideal), ndcgAt10: dcg/Math.max(1,ideal), usedUsers };
  }
  ...
  try{
    ...
    body.innerHTML =
      `<p>Validation users sampled: ${fmt(uPick.length)} (evaluated=${fmt(baseM?.usedUsers||deepM?.usedUsers||0)}, negatives=${sampleNeg})</p>`+
    ...
    console.error(e);
    body.textContent = 'Metric computation failed (likely due to item-id mismatch). Try reloading data and training again.';
  }
}
