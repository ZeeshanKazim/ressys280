...
class TwoTowerModel {
  constructor(numUsers, numItems, embDim=32, opts={}){
    ...
    // Variables (no explicit names → avoids duplicate-name crash on re-train)
...
class DeepTwoTowerModel {
  constructor(numUsers, numItems, embDim=32, tagDim=200, opts={}){
    ...
    this.itemTagMat = null; // [numItems, tagDim]
  }
  ...
  trainStep(uIdx, iIdx){
    return tf.tidy(()=>{
      const lossFn = () => {
        const U = this.gatherUsers(uIdx);                         // [B,D]
        const tagB = tf.gather(this.itemTagMat, iIdx);            // [B,K]
